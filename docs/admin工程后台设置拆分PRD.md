...
5. 不在本次拆分中重命名现有 API 路径，除非后续单独做 API 治理。
...
| 服务工作台和客户服务空间 | 服务执行场景 | 继续围绕“今天服务谁、怎么说、发什么、下一步做什么” |
...
个人使用和组织使用不要拆成两套账号系统，也不要拆成两套后端。建议统一为：
...
- 园长版：一个园长个人使用，本质是一个个人空间或个人增强空间。
- 园所版：一个幼儿园团队使用，本质是一个团队空间。
...
| 产品形态 | 资源归属 | 典型用户 | 成员能力 | 适合场景 | admin 展示 |
| --- | --- | --- | --- | --- | --- |
| 个人版 | 个人空间 | 老师、招生顾问、个人使用者 | 默认 1 人，可限制邀请 | 个人知识库、个人助理、个人资料整理 | 只展示轻量空间设置、知识库、智能体 |
| 园长版 | 园长个人空间 | 园长、校长、负责人 | 默认 1 人，可升级为园所空间 | 园长个人沉淀招生话术、家长沟通、园所经营资料 | 展示园长工作台和有限 admin |
| 园所版 | 园所团队空间 | 园长、招生主管、老师、运营 | 支持成员、角色、邀请、审计 | 园所统一知识库、招生 SOP、服务话术、渠道发布 | 展示完整空间 admin |
| 集团版 | 多园所组织 | 集团总部、区域负责人、多个园所 | 组织成员是园所空间，不是单个人 | 多园区模板、总部知识库下发、跨园共享 | 展示组织治理、共享、跨空间统计 |
...
| 智能体配置 | 支持轻量配置 | 支持招生和服务助理配置 | 支持团队智能体管理 | 支持模板化下发 |
...
### 15.6 Admin 工程里的差异化展示

admin 工程不应该给所有空间展示同一套重后台菜单，而应根据 `edition_code` 和权限裁剪。
...
| 开通页改造 | P0 | 支持个人、园长、园所、加入已有空间 |
| Admin 菜单裁剪 | P0 | 按版本、权限、能力开关生成导航 |
...
2. 个人版用户进入 admin 时，看不到成员、API Key、组织治理等不适用模块。
3. 园长版用户能使用园长相关知识库和智能体，并能发起升级为园所版。
4. 园所版 Owner 能邀请成员、分配角色、查看审计、管理园所知识库。
