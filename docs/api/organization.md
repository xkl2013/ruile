...
> 当前新建组织在产品界面中称为“团队空间”，并限制在创建者所属企业空间内。知识库访问以显式发布关系为准，智能体共享不会自动授予其绑定知识库权限；旧 `/organizations` 路径和历史跨空间数据仅作为兼容能力保留。详细规则见 [团队空间实现方案](../团队空间实现方案.md)。

组织（Organization，又称"空间"）是 睿乐大脑的多空间协作单元。一个用户可以创建/加入多个组织，并以 owner / admin / editor / viewer 的角色参与；知识库与智能体可以共享到组织内，组织成员根据角色获得相应的访问权限。
