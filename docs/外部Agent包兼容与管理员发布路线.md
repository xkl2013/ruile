...
> 文档状态：技术实施附录，保留包导入与发布治理设计
> 更新日期：2026-09-17
> 适用项目：ruile
> 主文档：[可扩展专家 Agent 服务平台 PRD](./服务Agent与卡片报告架构设计.md)
> 本文范围：说明外部 Agent 包如何由后台管理员导入、适配、评测、发布和回退；普通用户不具备导入或安装 Agent 的权限
> 优先级说明：本文第 8 章是第一版导入框架路线；当前已完成能力和下一阶段研发顺序以主文档第 19 章为准
...
本章保留最初的能力拆分，便于追溯导入框架设计。AgentRun、ZIP 导入、基础 Registry、发布绑定和后台测试已经部分或全部落地；后续优先实施 Expert Workflow V2、Skill 实际加载和质量门禁，详见主 PRD。
