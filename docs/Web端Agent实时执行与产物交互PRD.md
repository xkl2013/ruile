...
1. 不做普通用户导入、安装或修改专家包。
2. 不做专家与用户身份的长期绑定。
3. 不做专家与服务模块的自动绑定，也不把 AgentRun 结果写入服务空间、服务待办或服务卡片。
4. 不做 CRM、工单、日历、IM 等外部系统写入。
5. 不做高并发弹性扩缩容和复杂计费。
6. 不做公开分享链接和匿名访问。
7. 不要求所有 Skill 在本期都变成可执行脚本。
8. 不把每一轮普通聊天都自动保存成长期产物。
9. 不替换当前聊天系统，只增加通用 Agent 运行和产物交互能力。

### 2.3 当前实施边界

本阶段只建设通用 Agent 基座，所有能力围绕 `AgentRun`、`AgentThread`、`AgentArtifact` 和事件流展开：

- `AgentRun`：统一任务状态、阶段、输入快照、质量结果、父子版本和取消能力。
- `AgentThread`：通过 `thread_id` 组织同一专家任务的多轮执行，支持继续追问和上下文恢复。
- `AgentArtifact`：统一产物引用、预览、下载和 HTML、文本、图片、PDF、音视频等格式分发。
- `AgentRunEvent`：统一队列、执行、等待输入、完成、失败和取消事件，前端通过 SSE 恢复。
- Agent 包、Skill、模型和沙箱的调用继续通过现有专家执行链路接入，但不在本阶段新增服务领域规则。

服务模块相关的专家绑定、服务空间写入、服务待办生成、服务卡片落库、客户或工作画像关联均暂缓，后续按新的服务模块设计单独实现。当前“结果摘要卡片”仅是聊天中的 Agent 输出，不代表服务实体。
...
- 平台默认最多进行一轮模型需求澄清，每轮最多 4 个问题。
- 首轮只询问会改变交付类型、核心目标、目标对象、总体范围或硬性边界的宏观变量。
- 人员姓名、品牌型号、精确时间和数量、可选偏好、实现参数等微观细节不得阻断首版交付，应进入假设或待确认项。
- 只有专家包通过 `required_inputs` 显式声明的硬性必填字段，才允许在首轮之后继续阻断运行。
- 平台澄清轮次和阻断规则优先于专家正文中的一般询问习惯，避免外部专家包触发无限追问。
- 优先使用单选或多选。
- 单题单选可以点击即提交。
...
`clarification_policy.max_rounds` 和 `clarification_policy.max_questions` 最终受平台上限约束。专家正文可以说明希望确认的信息，但不能绕过平台的宏观优先、默认单轮澄清和微观细节不阻断策略。
...
- 成功结果下方提供“补充执行细节”“整理成行动清单”“调整得更简洁”“重新生成一版”等快捷动作。
- 快捷动作通过 `regenerate` 创建带 `parent_run_id` 的子运行，不覆盖上一版卡片和产物。
- 子运行读取上一版 `report_markdown` 作为修订基线，并沿用原始请求、已确认答案和 Requirement Snapshot 信息。
- 已存在上一版正式报告时，不重复发起模型需求澄清；专家包显式声明但上一版仍未满足的 `required_inputs` 除外。
- 失败结果提供“重试本次任务”，重试仍创建子运行并保留失败记录。

当前版本同时实现轻量解释型追问：

- 成功结果下方提供“为什么这样安排”“预算是怎么算的”“安全风险怎么判断”等预置动作。
- 解释型追问通过 `/api/v1/expert-packages/published/runs/{run_id}/follow-ups` 创建独立 `AgentRun`，复用已发布专家、Skill 快照、上一版报告和事件流。
- 解释型追问只返回聊天消息，不生成服务卡片、HTML/PDF 或其他正式产物。
- 解释型追问结果可以和普通专家运行一样恢复，并按会话保存在前端历史快照中。
- `ServiceThread`、跨多轮对话摘要、版本差异查看和自然语言追问分类仍属于后续增强，不阻断当前聊天闭环。
...
截至 2026-09-20，本期 P0 基础闭环已完成验收：

- 已发布专家可以从 Web 聊天窗口发起真实 `AgentRun`，并展示排队、执行、需求澄清、规划、撰写、质量审查、修订和完成等事件流。
- 事件流在执行结束后自动折叠，用户仍可展开查看事件名称、阶段摘要、输入和返回结果。
- 结果卡片只展示标题、摘要和下一步动作；详细内容通过产物入口查看。
- 结构化报告会生成 HTML 主产物和 Markdown 辅助产物，HTML 可在右侧面板进行服务端预览，产物支持下载。
- 页面刷新后可以恢复已完成运行、结果卡片和双产物；运行控制覆盖取消、失败重试、纠偏子运行和陈旧运行超时恢复。
- 已完成服务层、仓储/容器/处理器回归测试，以及前端类型检查和生产构建。

本版本明确暂缓：

- `AgentArtifact` 独立业务实体、产物版本中心和长期资产生命周期。
- OSS 持久化、公开分享链接、历史产物中心和跨会话资产检索。
- 服务绑定、服务空间写入、服务待办落库和外部系统联动。
- 高并发弹性治理、完整脚本型 Skill 沙箱和评测集自动运营。

## 22. 专家指定与自动路由

### 22.1 产品决策

聊天输入框同时支持两种专家选择方式，不强制用户只能手动选择或只能使用自动路由：

1. **指定专家**
   - 用户在输入框下方打开专家选择器，指定一个已发布专家。
   - 输入框下方显示专家头像和名称，作为本次提问的固定执行对象。
   - 用户明确指定后，系统不得因为路由器判断改变而自动换专家。
   - 专家不可用、版本被停用或权限校验失败时，向用户展示结构化替代选项，不静默改派。

2. **路由选择专家**
   - 用户保持“自动匹配”状态，不指定具体专家。
   - 系统根据问题意图、领域、任务类型、产物要求和已发布专家能力进行路由。
   - 路由成功后，在事件流中展示“已匹配专家：xxx”和简短匹配理由。
   - 用户可以在执行开始前点击“更换专家”，切换为指定专家。
...
+    自动匹配⌄       [专家头像] 销售教练       模型/权限/停止控制
```

其中：

- `自动匹配` 是默认路由模式。
- 选择具体专家后，`自动匹配` 替换为专家名称。
- 专家名称以可取消的选择项展示，取消后恢复自动匹配。
- 普通用户只能看到已发布专家，不能从本地导入或修改专家包。
...
group
  -> 预留给专家团编排，不进入本次自动路由首版
```

本阶段只实现 `auto` 和 `manual`。`group` 后续再实现，避免首版自动路由同时引入多专家并发、结果合并和成本控制问题。

### 22.3 路由输入

路由器只能从管理员已发布版本中选择，候选专家索引至少包含：
...
- 用户任务意图和领域。
- 任务类型，例如方案生成、分析、改写或解释。
- 是否需要特定产物，例如 HTML、Markdown、图片或表格。
- 是否需要指定 Skill 或能力。
- 是否属于高风险领域。
- 当前会话已经固定的专家和上下文。
...
第一版可以将路由结果存入现有 `AgentRun` 的运行元数据，不新增独立路由表：

```json
{
  "route_mode": "auto",
  "selected_expert_id": "kindergarten-activity-planner",
  "selected_version": "1.0.0",
  "confidence": 0.94,
  "candidate_experts": [
    {
      "expert_id": "kindergarten-activity-planner",
      "confidence": 0.94
    }
  ],
  "routing_reason": "幼儿园活动策划、亲子运动会、方案生成",
  "requires_confirmation": false,
  "user_confirmed": false
}
```

路由结果必须和实际执行的 `expert_id`、版本号一起保存，不能只保存路由器的推荐结果。
...
| 路由结果 | 默认行为 |
| --- | --- |
| 高置信度，建议 `>= 0.85` | 自动选择并开始执行 |
| 中置信度，建议 `0.60-0.85` | 展示 2-3 个专家选项，用户点击确认 |
| 低置信度，建议 `< 0.60` | 展示一个结构化澄清问题，暂不执行 |
| 高风险任务 | 即使高置信度，也要求用户确认专家 |
| 用户已指定专家 | 跳过自动路由，直接执行指定专家 |

路由器不得通过自由文本反复询问“你想选择哪个专家”。需要用户参与时，优先展示专家选项按钮、领域选项或任务类型选项。
...
路由器的内部评分、完整候选列表和失败原因只对管理员诊断可见；普通用户只展示当前阶段、已选专家和必要的简短理由。

### 22.7 失败与人工覆盖

- 无候选专家：进入普通聊天或展示“暂未找到合适专家”的结构化提示。
- 候选冲突：展示最多 3 个候选专家，不自动随机选择。
- 路由器异常：保留用户问题，允许用户手动指定专家后重试。
- 专家执行失败：沿用现有 AgentRun 失败重试，不重新路由，除非用户明确选择更换专家。
- 用户点击“更换专家”：创建新的运行版本，保留原运行和路由决策。

## 23. 下一阶段研发计划

### 23.1 P0.5：自动路由单专家闭环

目标：在现有 AgentRun 基座上，增加“自动匹配”和“指定专家”并行存在的能力。

功能点：

- 输入框增加 `自动匹配` 默认状态和专家选择器。
- 支持指定已发布专家，并显示专家头像、名称和可取消选择项。
- 发起任务接口支持 `route_mode=auto|manual`。
- 新增 `ExpertRouter` 服务，从已发布专家目录中筛选候选。
- 保存路由决策、候选专家、置信度和匹配理由。
- 事件流增加识别任务和匹配专家阶段。
- 支持执行前人工覆盖。
- 路由失败后可以直接切换专家重试。
- 不改服务模块，不写服务待办，不做专家身份绑定。

验收标准：

- 不选择专家时，系统可以自动调用正确的已发布专家。
- 指定“童创”后，无论路由器判断如何变化，都只执行“童创”。
- 自动路由失败时，用户可以通过按钮选择专家继续。
- 刷新页面后仍能看到实际执行的专家和路由模式。
- 事件流中可以区分“正在匹配专家”和“正在执行专家”。
...
- 建立专家路由固定测试集。
- 覆盖明确领域、跨领域、模糊问题和无匹配问题。
- 增加高风险领域人工确认规则。
- 增加错误路由、误调用和候选冲突诊断。
- 管理后台展示路由命中率、人工改选率和失败原因。

验收标准：

- 明确领域问题不能路由到错误专家。
- 用户手动指定专家时不能被自动改派。
- 未发布、停用或无权限专家不能成为路由结果。
- 高风险任务不能绕过人工确认。
- 所有自动路由结果可以追溯到专家版本和路由理由。

### 23.3 P1：专家团与多专家编排

暂不与单专家自动路由同时开发。后续再实现：
...
专家团不是本版本自动路由的默认行为，必须由用户明确选择或由管理员配置。

### 23.4 建议排期

以下为小团队的建议排期，实际时间根据人员投入调整：

| 时间 | 工作内容 | 交付物 |
| --- | --- | --- |
| 2026-09-21 至 2026-09-22 | 专家目录索引、路由协议、运行元数据设计 | 路由协议和数据结构 |
| 2026-09-23 至 2026-09-25 | 后端 `ExpertRouter`、自动/手动运行入口、路由事件 | 可执行的自动路由链路 |
| 2026-09-26 至 2026-09-28 | 输入框专家选择器、自动匹配状态、人工覆盖和错误提示 | 前端交互闭环 |
| 2026-09-29 至 2026-09-30 | 固定路由测试集、管理员诊断和灰度验收 | P0.5 验收报告 |
...
- 专家与服务领域自动绑定。
- OSS 保存、公开分享和长期资产管理。
- 独立 `AgentArtifact` 实体和历史产物中心。
- 专家团、多专家并行和跨 Agent 结果合并。
- 复杂脚本型 Skill、外部系统写入和高并发弹性治理。
