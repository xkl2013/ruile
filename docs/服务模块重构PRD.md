...
**服务空间归属边界（D24）**：服务空间是**聊天记录、会话和产出物的唯一业务归属边界**。用户进入「秋季招生咨询」后创建的会话，其全部用户消息、助手回复、工具过程与引用都属于该服务；由这些会话触发的 Agent 运行与报告 / 文件产出物也属于该服务，只能在该服务的侧栏、会话历史和产物列表中出现。聊天消息不重复增加 `service_id`，而是通过 `messages.session_id → sessions.service_id` 继承归属；产物直接记录 `service_id`，并通过 `run_id → agent_runs.thread_id → sessions.id` 追溯来源会话。一个会话、消息或产出物不能同时属于多个服务；全局聊天（`service_id IS NULL`）不归属于任何服务。跨服务复用只能复制内容并在目标服务创建新会话 / 新产出物，不得移动或混用历史记录。详见 §4.1、§5.3、§10.3。
...
| D24 | 服务空间内容归属 | **服务空间是会话、聊天消息与产出物的业务归属边界。** 会话创建时必须写入当前 `service_id`；会话内所有 `messages` 通过 `session_id → sessions.service_id` 继承该服务归属；Agent 运行与产出物必须与当前服务一致。以「秋季招生咨询」为例，在该空间产生的咨询会话、用户 / 助手聊天记录、运行过程和报告 / 文件都只属于「秋季招生咨询」 | `sessions.service_id` 是会话归属真源；`messages` 不新增 `service_id`，通过所属会话继承；`agent_runs.service_id` 与 `thread_id` 必须指向同一服务会话；`service_artifacts.service_id` 直接记录归属，并通过 `run_id` 追溯来源会话。禁止跨服务读取、挂接、移动或混用；全局聊天 `service_id IS NULL` 保持独立 |
...
| **ServiceChatMessage（服务聊天记录）** | 服务会话内产生的用户消息、助手回复、工具过程、知识引用与附件。消息不单独存 `service_id`，通过 `messages.session_id → sessions.service_id` 继承服务归属；全局消息仍归属于 `service_id IS NULL` 的全局会话 | 现有 `messages` 表（不新建、不增加 `service_id`） |
...
| `messages` | **复用，不改表** | 通过 `session_id` 关联 `sessions`，从父会话继承 `service_id`；服务上下文的读写必须先校验父会话归属，不允许跨服务挂接消息 |
...
| `messages` | 复用 | **不新增 `service_id`**；消息通过 `session_id` 继承父会话的服务归属（D24） |
...
> **聊天记录的归属（D24）**：`messages` 表不新增 `service_id`。一条消息的服务归属唯一由父会话决定，即 `messages.session_id → sessions.service_id`。服务上下文下的消息读取、删除、续写和搜索都必须先校验 `sessions.service_id = 当前服务`；不能只凭 `message_id` 或 `session_id` 的存在就放行。这样可以避免会话归属与消息归属出现两份真源，也保证「秋季招生咨询」下的全部咨询记录不会出现在其他服务中。全局会话的 `service_id IS NULL` 消息继续按平台原有语义处理。

> **产出物的来源链（D24）**：`service_artifacts.service_id` 是产出物的直接服务归属；若产出物来自 Agent 运行，则必须同时满足 `service_artifacts.run_id → agent_runs.service_id`，且 `agent_runs.thread_id → sessions.id` 的会话也属于同一 `service_id`。因此产出物既能按服务空间快速过滤，也能回到具体会话与运行记录；任何一层归属不一致都应拒绝写入。
...
| FR-203 | 事件 | 当用户在服务空间内发起 Agent 运行时，系统应把 `service_id` 与（可选的）`subject_id`、`thread_id` 写入 `AgentRun`；`thread_id` 必须指向当前服务的 `sessions.id`，不新增 `AgentRun.session_id` |
| FR-204 | 事件 | 当 Agent 运行产出报告或文件时，系统应在 `service_artifacts` 写入索引记录，并将 `service_id` 写入 `AgentWorkDoc`；产物的 `service_id`、来源运行的 `agent_runs.service_id` 与 `agent_runs.thread_id → sessions.service_id` 必须一致 |
...
| FR-216 | 状态 | 服务会话只能归属于一个服务：创建服务会话时 `sessions.service_id` 必须等于当前服务；已归属服务的会话不得被改挂到另一个服务；`service_id IS NULL` 的全局会话不出现在任何服务空间 |
| FR-217 | 普遍 | 服务空间内的聊天消息必须通过 `messages.session_id → sessions.service_id` 继承服务归属；消息读取、删除、续写、搜索与清空均须校验父会话属于当前服务，不得仅凭消息 ID 或会话 ID 跨服务访问 |
| FR-218 | 事件 | 在服务会话中产生的用户消息、助手回复、工具过程、知识引用和附件都必须留在该会话下；调用平台既有消息接口时，服务模块必须透传当前 `service_id` 或执行等价的父会话归属校验 |
| FR-219 | 事件 | 服务会话产生的报告 / 文件必须写入 `service_artifacts.service_id`，并记录来源 `run_id`；产物列表、预览、下载与搜索只能返回当前服务的产物，不得把其他服务或全局产物混入 |
| FR-220 | 状态 | 跨服务复用历史聊天记录或产出物时，不得直接修改原记录的 `service_id` 或把原消息挂接到目标会话；只能复制内容，在目标服务创建新的会话消息或新的产出物，并保留来源引用（如有） |
...
| `GET` | `/services/:service_id/sessions/:session_id` | viewer+ | 会话详情（含该会话全部聊天消息；消息体沿用平台既有读取接口，但必须校验 `sessions.service_id`） |
...
> **服务聊天记录的统一归属校验（D24）**：平台消息接口虽然仍以 `session_id` 为路径参数，但从服务空间调用时必须带入当前 `service_id` 上下文，或在服务层先加载并校验父会话。`messages` 不新增 `service_id`；服务空间的会话详情、历史加载、删除、清空、续写与消息搜索均不得绕过这层校验。
...
1. **当前服务是所有新内容的写入上下文。** 用户在「秋季招生咨询」空间内首次发言时，系统创建的 `sessions` 必须写入该服务的 `service_id`；之后产生的用户消息、助手回复、工具过程、知识引用和附件都通过该会话归属于「秋季招生咨询」。
2. **聊天记录不跨空间展示。** 服务侧栏、会话列表、会话详情、历史搜索和消息操作都必须先校验 `sessions.service_id = 当前服务`。消息表不重复存 `service_id`，防止会话归属与消息归属出现两份真源。
3. **产出物沿用同一条归属链。** 当前会话触发的运行写入同一服务的 `agent_runs.service_id`，运行产生的报告 / 文件写入 `service_artifacts.service_id`，并记录来源 `run_id`；产物卡、产物列表、预览、下载都只在该服务上下文内返回。
4. **不允许一条内容多归属。** 会话、消息和产出物只能属于一个服务；全局聊天（`service_id IS NULL`）不属于任何服务，也不应出现在任一服务空间。
5. **跨服务复用采用复制，不做移动。** 若需要把「秋季招生咨询」中的内容用于另一个服务，应在目标服务创建新会话 / 新消息 / 新产出物，并保留来源引用；不得直接修改原会话的 `service_id`，也不得把原消息挂到目标会话。

**示例**：在「秋季招生咨询」中询问“本周哪些家长需要回访”，该问题、Agent 回复、执行步骤以及生成的《秋季招生回访表.xlsx》都属于「秋季招生咨询」。切换到其他服务时，这些内容不会出现在该服务的会话列表、聊天历史或产物列表中。
...
>
> 结论：**会话是"迁移之后新产生的工作"的载体。** 迁移只保证新写入路径正确（`service_id` 必填、`thread_id = session_id`），历史数据维持原状，不做人工归并。若某租户确需在服务内复用历史内容，走**产品化的"复制到服务"操作**（本期不做，见 Q33）：在目标服务创建新会话并复制必要内容，不修改原全局会话及其消息归属，也不做批量脚本。
...
> 本模块按四个产品版本交付。每个版本都必须可独立部署、可回滚，并完成对应验收；版本内部可以并行开发，但不得把后续版本的数据模型或 UI 依赖提前暴露给用户。

| 版本 | 版本目标 | 本版本实现范围 | 依赖与验收门槛 | 明确不纳入 |
| --- | --- | --- | --- | --- |
| **V1 · 服务空间基础版** | 建立服务实体、服务上下文和最小权限边界，让用户可以创建并进入多个相互隔离的服务 | ① `services`、最小 `service_members` 与服务级 ACL；② 服务列表、创建、详情、暂停 / 启用 / 归档；③ `service_id` 归属改造：服务对象、事项、工作文档、草稿、运行预留字段；④ 存量画像迁移与 ID 复用；⑤ 服务空间路由、基础空态和权限降级；⑥ 全局聊天保持原语义，`sessions.service_id IS NULL` 的数据不回填 | 迁移在预发环境完成演练并可回滚；同一用户可创建多个服务；服务 A 的对象、事项和配置不能被服务 B 查询；owner / admin / editor / viewer 权限校验通过；对应 AC-1、AC-2、AC-4、AC-5、AC-M1～M9 | 不开放服务内 Agent 运行、会话列表、产出物、完整事项状态机和邀请协作 |
| **V2 · 会话与聊天版** | 把服务空间变成可连续工作的聊天空间，聊天记录严格归属于当前服务 | ① `sessions.service_id`、`expert_ref`、`expert_name`；② 服务级会话列表、创建、详情、重命名、置顶、删除；③ `messages.session_id → sessions.service_id` 的归属校验；④ 复用既有聊天组件，支持服务空间内消息流、历史加载、清空与续写；⑤ 应用侧栏“服务 → 会话”三级结构、手风琴与当前会话直达；⑥ 服务头新建会话入口与归档态只读行为 | 在“秋季招生咨询”创建会话并发送多轮消息，所有消息只能从该服务会话读取；服务 A 不能读取 / 删除 / 搜索服务 B 的消息；全局会话不出现在服务侧栏；会话与运行记录不混为一张列表；对应 D14、D21、D24、FR-210～218、AC-16～19、AC-F21～F22、AC-F26 | 不生成正式报告 / 文件；不开放跨服务复制；不在本版本承诺 Agent 运行步骤和产物列表 |
| **V3 · Agent 基座与产出物版** | **先优化底层 Agent，再打通“服务会话 → Agent 运行 → 产出物 → 服务内回看”闭环。** 服务层不得绕过 Agent 基座直接拼接产出物 | **阶段 A：Agent 基座优化**——① 统一 Workflow Runner：需求澄清 / 规划 / 生成 / 审查 / 修订 / 制品化；② 统一 `agent_result_v1` 与 Deliverable Contract，支持一个主要产物和多个辅助产物；③ `AgentArtifact` / `AgentArtifactVersion` / `ResourceBinding` 的逻辑模型、不可变版本、临时生命周期与资源引用；④ Agent / Skill / 模型 / 提示词版本锁定；⑤ 运行步骤、幂等、重试、停止、续传和失败诊断；⑥ 输出结构校验、质量审查和有限次数自动修订。**阶段 B：服务接入**——⑦ `AgentRun.service_id` 与 `thread_id = sessions.id`；⑧ 服务内按专家发起运行、实时展示步骤；⑨ `service_artifacts` 作为服务域索引，记录 `service_id`、`run_id`、`resource_id` 与当前版本；⑩ 对话产物卡、服务头产物入口、列表 / 预览 / 下载；⑪ 产出物来源链、跨服务拒绝写入和存储配额记账 | **阶段 A 未通过技术闸门时，不进入阶段 B，也不允许在服务层新增临时产物协议。** 阶段 A 至少通过：多种产物类型统一返回、输出协议校验、失败可恢复、重新生成产生新版本且不覆盖旧版本、Agent / Skill 版本可追溯；阶段 B 再验证“秋季招生咨询”中运行、会话、产物三者归属一致，其他服务不可见，`resources.size` 与存储账本可对账，配额不足在开工前拦截；对应 D22、D24、FR-203～204、FR-219～220、FR-318～320、AC-3、AC-F23、AC-F27 | 不实现完整产物中心的外部分享、复杂卡片编排和跨租户能力；不将所有历史全局运行自动归入服务；不允许服务层直接写 Agent 业务结果表来替代基座协议 |
| **V4 · 协作与配置完整版** | 补齐服务作为团队工作空间的协作、专家配置、事项配置和兼容收口能力 | ① 完整成员邀请、离职记录与协作权限；② 专家绑定及岗位级配置；③ 服务级知识库范围与内置技能配置；④ 服务对象工作台；⑤ 事项状态机、流转规则、字段选项、多负责人、子任务、评论、操作历史；⑥ 服务消息（复用 IM）与右侧面板；⑦ 服务头设置、概览、产物 / 事项统一入口；⑧ 旧接口兼容期结束后的路由和旧表清理；⑨ 文档、Swagger、埋点和运维脚本同步 | 四个版本的端到端回归通过；成员协作、专家收窄、事项流转与配置引用保护通过；服务设置不突破服务 ACL；旧路由调用量为 0 后再清理；对应 D10、D18、D23、FR-301～309、FR-321～324、AC-F1～F25、AC-C1～C3 | 不纳入跨租户共享、服务对外计费、完整产物中心、周期性重复事项和历史全局会话自动迁移 |

**版本之间的硬依赖：**

1. **V1 → V2**：没有稳定的 `service_id` 与服务级 ACL，不能创建服务会话，也不能保证聊天记录隔离。
2. **V2 → V3-A**：服务会话提供 Agent 的输入上下文，但 V3-A 必须先完成底层 Workflow Runner、统一输出协议和版本化产物能力；不能直接从服务页面拼接一次性文件。
3. **V3-A → V3-B**：只有 Agent 基座技术闸门通过后，才把运行和产物接入服务空间；服务层只做服务归属、权限过滤和展示，不复制 Agent 产出协议。
4. **V3 → V4**：协作和配置入口必须建立在已有的服务归属链上；服务级知识库 / 技能授权不能绕过运行与产物的归属校验。
5. **任一版本都不得回填全局历史会话**：存量 `sessions.service_id IS NULL` 的聊天继续保持全局语义；需要复用时，按 D24 复制到目标服务并创建新会话。

**推荐发布节奏：**

| 发布批次 | 目标 |
| --- | --- |
| V1 | 先完成数据库迁移、服务 CRUD 和服务隔离，作为后续三版的稳定地基 |
| V2 | 优先让用户在服务内连续聊天并能从侧栏找回历史工作 |
| V3 | 先完成 Agent 基座优化与技术闸门，再交付服务内 Agent 运行和产出物闭环 |
| V4 | 补齐团队协作、配置管理、兼容清理和完整回归 |

**关键路径**：V1 → V2 → V3-A（Agent 基座）→ V3-B（服务接入）→ V4。迁移与归属校验是 V1 的最大风险，Agent 输出协议、版本与恢复能力是 V3 的最大风险；任一闸门未通过，都暂停服务产出物接入，不用临时协议掩盖底层能力缺口。
...
| AC-F26 | 服务空间聊天记录归属（D24、FR-216~218） | 在「秋季招生咨询」空间内创建会话并发送用户消息，验证 `sessions.service_id` 正确；该会话产生的用户 / 助手消息均通过 `messages.session_id` 关联到该会话；服务 A 的会话详情、历史加载、搜索和清空接口不能读取或操作服务 B 的会话消息；`service_id IS NULL` 的全局会话不出现在任何服务空间 |
| AC-F27 | 服务空间产出物归属（D24、FR-219~220） | 在「秋季招生咨询」的会话内完成一次 Agent 运行并生成报告 / 文件，验证 `service_artifacts.service_id`、`agent_runs.service_id` 与来源会话的 `sessions.service_id` 一致，且 `run_id` 可追溯；其他服务的产物列表、预览、下载和搜索均不可返回该产物；尝试直接把会话或产物改挂到另一个服务时请求被拒绝 |
...
| Q8 | 一个会话能否跨服务 | 影响 `sessions.service_id` 语义 | **已定（D24）**：不可跨服务。一个会话只能有一个 `service_id`；跨服务使用必须创建新会话，原会话与原消息不可直接移动或混用 |
...
| Q33 | **存量全局会话能否"移动到服务"** | 迁移刻意不给存量 `sessions` 回填 `service_id`（§11.2）。但用户手上有真实的历史会话工作，可能希望认领到某个服务下 | **当前不支持直接移动**（D24）：存量全局会话继续属于个人聊天；如未来需要复用，采用"复制内容到目标服务并创建新会话"的方式，不修改原会话及其消息的归属，也不做批量脚本 |
...
| 2026-09-22 | v0.23 | **D24 落地：服务空间成为会话、聊天记录与产出物的唯一业务归属边界。** 明确「秋季招生咨询」空间内产生的会话、用户 / 助手消息、工具过程、知识引用、运行记录与报告 / 文件都只属于该服务；`messages` 不新增 `service_id`，通过 `messages.session_id → sessions.service_id` 继承归属；`service_artifacts.service_id` 直接记录产出物归属，并通过 `run_id` 追溯来源会话与运行。新增 FR-216~220、AC-F26 / AC-F27；新增 §10.3「内容归属规则」、§4.1 / §4.2 / §4.3 / §5.1 / §5.3 的数据模型与迁移口径；API 增加服务上下文下的消息归属校验；Q8 收口为"会话不可跨服务"，Q33 改为不支持直接移动历史会话。 |
| 2026-09-22 | v0.24 | **制定四版本实现路线**：V1 服务空间基础版（服务实体、迁移、ACL 与隔离）；V2 会话与聊天版（会话、消息归属与侧栏入口）；V3 Agent 与产出物版（运行、产出物、存储计量闭环）；V4 协作与配置完整版（成员、专家、知识库 / 技能、事项配置、兼容清理）。§12.3 改为四个可独立发布、可回滚的版本，补充版本硬依赖、验收门槛、排除范围与推荐发布节奏。 |
| 2026-09-22 | v0.25 | **V3 增加 Agent 基座技术闸门**：先优化底层 Workflow Runner、统一 `agent_result_v1` / Deliverable Contract、产物版本与生命周期、运行步骤、幂等、重试、停止、续传和质量审查；底层 Agent 未通过技术验收前，不进入服务层产出物接入，不允许用临时协议或服务层直接写业务结果表来替代。V3 拆为 V3-A（Agent 基座）与 V3-B（服务接入），并同步修订 §12.3 的版本依赖、验收门槛和关键路径。 |
| 2026-09-22 | v0.26 | **开始落地 V3-A Agent 基座优化**：`agent_result_v1` 的产物统一补齐逻辑 `artifact_id`、不可变 `version_id` / `version`、来源 `run_id`、生命周期、预览 / 下载能力、创建时间和元数据；Agent 完成时统一规范化旧版输出，并通过 `ARTIFACT_VERSION_CREATED` 事件广播产物版本。重新生成会继承父产物逻辑 ID、递增版本并保留旧运行结果；失败事件补充失败阶段、尝试次数、是否可重试和是否正在自动重试。已补充类型、服务层与运行回归测试；**V3-B 的服务归属、`service_artifacts` 索引、配额记账和服务内产物列表尚未开始，继续等待基座闸门验收**。 |
| 2026-09-22 | v0.27 | **按四版本路线开始实现服务模块**：① V1 新增服务空间实体、成员 ACL、状态机、专家绑定、服务概览与服务级路由；② V2 将 `sessions.service_id`、`expert_ref`、`expert_name` 接入服务会话，服务侧栏改为“服务 → 会话”三级入口，聊天历史、重命名、置顶、清空、删除和流式消息均通过服务上下文校验；③ V3-B 接入 `AgentRun.service_id` 与 `thread_id = sessions.id`，运行完成后把规范化 Agent 产物写入 `service_artifacts`，提供服务级运行 / 步骤 / 事件 / 产出物接口，并校验运行、会话、服务三者归属一致；④ 删除旧的 `/api/v1/service` 服务模块及其 profile / 日报 / memory extraction 依赖，不保留旧模块兼容层；⑤ 前端服务工作区、菜单和聊天头部改用新服务空间 API。已补充 SQLite / PostgreSQL 迁移、服务隔离回归测试、AgentRun 服务边界测试与前端构建验证。**V4 的事项状态机、完整邀请协作配置和存储账本预留 / 提交仍待继续实现。** |
