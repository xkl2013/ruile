...
//
// Chunks are KB content. Mutating routes use the shared KB-access guard so
// source-tenant Admin+/creator and shared-space editor/admin can edit them,
// while shared-space viewers remain read-only.
...
//
// Knowledge rows are KB content. Mutating routes rely on KBAccessWrite so
// shared-space editor/admin can upload, update, reparse, and delete files;
// shared-space viewers still fail the Editor-level check. Cross-:id batch
// operations validate their KB access inside the handler because the KB id
...
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler, g *rbacGuards) {
	...
	kRead := k.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		// routes; /move and /batch-delete are not declared for API keys.
		...
		// Batch / cross-KB write ops validate KB access in the handler because
		// the KB id comes from the request body. Viewer-only members still fail
		// the handler's KBAccessWrite/Editor check.
...
func RegisterFAQRoutes(r *gin.RouterGroup, handler *handler.FAQHandler, g *rbacGuards) {
	...
	// 等价于修改 KB 内容，统一由 KBAccessWrite 判定：源空间 Admin+/创建者、
	// 共享空间 editor/admin 可写，viewer 只读。
...
//
// Tags are KB metadata/content: Viewer reads, KBAccessWrite controls writes.
// That keeps same-tenant owner/admin and shared-space editor/admin behaviour
// aligned with document, FAQ, chunk, and wiki content routes.
func RegisterKnowledgeTagRoutes(r *gin.RouterGroup, tagHandler *handler.TagHandler, g *rbacGuards) {
	...
	// 行为，统一由 KBAccessWrite 判定共享空间 editor/admin 是否可写。
...
// RegisterSMSAuthRoutes registers SMS verification login routes.
func RegisterSMSAuthRoutes(r *gin.RouterGroup, handler *handler.SMSAuthHandler) {
	if handler == nil {
		return
	}
	r.POST("/auth/sms/send-code", handler.SendLoginCode)
	r.POST("/auth/sms/login", handler.Login)
}
...
// chunk / tag read routes. Content mutations and maintenance actions use
// KBAccessWrite so shared-space editor/admin can manage wiki content while
// viewer-only members stay read-only.
func RegisterWikiPageRoutes(r *gin.RouterGroup, wikiHandler *handler.WikiPageHandler, g *rbacGuards) {
	...
	wikiRead := wiki.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		wiki.POST("/pages", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.CreatePage)
		wiki.PUT("/move-page", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.MovePage)
		wikiRead.GET("/pages/*slug", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.GetPage)
		wiki.PUT("/pages/*slug", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.UpdatePage)
		wiki.DELETE("/pages/*slug", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.DeletePage)

		// Folder tree (directory nodes)
		wikiRead.GET("/folders", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.ListFolders)
		wiki.POST("/folders", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.CreateFolder)
		wiki.PUT("/folders/:folder_id", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.UpdateFolder)
		wiki.DELETE("/folders/:folder_id", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.DeleteFolder)
		...
		wiki.POST("/rebuild-links", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.RebuildLinks)
		wikiRead.GET("/lint", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.Lint)
		wiki.POST("/auto-fix", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.AutoFix)

		// Issues
		wikiRead.GET("/issues", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.ListIssues)
		wiki.PUT("/issues/:issue_id/status", g.Viewer(), g.KBAccessWrite("kb_id"), wikiHandler.UpdateIssueStatus)
	}
}
