...
This directory contains a WeChat Mini Program plugin for 睿乐大脑. It gives mobile users a lightweight entry point to:

- configure a 睿乐大脑 API endpoint and tenant API key;
- list available knowledge bases;
- import a URL into a selected knowledge base;
- ask a selected knowledge base through 睿乐大脑 knowledge chat.
...
3. Open the **Settings** tab and fill in:
   ...
   - API Key from the 睿乐大脑 tenant settings page.
...
- WeChat DevTools may block `localhost` requests when URL validation is enabled. For local testing, either disable domain validation in DevTools or expose 睿乐大脑 through a HTTPS development domain.
- In production Mini Programs, add the 睿乐大脑 API domain to the Mini Program request domain allowlist.
