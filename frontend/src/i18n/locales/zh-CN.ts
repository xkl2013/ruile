export default {
  ...
  organization: {
    title: "共享空间",
    subtitle: "创建或加入共享空间，让多个园所空间互相协作，共享知识库与智能体",
    createOrg: "创建共享空间",
    createOrgShort: "新建",
    joinOrg: "加入共享空间",
    joinOrgShort: "加入",
    ...
    inviteCodeTip: "分享此邀请码给其它园所空间管理员，他们可以通过邀请码加入共享空间",
    ...
    deleteConfirmMessage: "确定要删除共享空间「{name}」吗？删除后所有参与空间将被移除，此操作不可撤销。",
    ...
    memberRemoved: "参与空间已移除",
    memberRemoveFailed: "移除参与空间失败",
    empty: "您还没有加入任何共享空间",
    ...
    detail: {
      myRole: "我的角色",
      removeMemberTitle: "移除参与空间",
      removeMemberConfirm: "确定要移除参与空间「{name}」吗？",
      removeMember: "移除参与空间",
    ...
    settings: {
      editTitle: "共享空间设置",
      detailTitle: "共享空间详情",
      myRoleDesc: "当前空间在此共享空间中的角色决定了权限范围",
      membersDesc: "查看哪些园所空间已加入本共享空间，并调整这些空间的共享权限。这里不是员工账号管理。",
      sharedDesc: "查看共享到本共享空间的所有知识库",
      noSharedKB: "暂无共享的知识库",
      noSharedKBTip: "知识库拥有者可以在知识库设置中将其共享到本共享空间",
      sharedAgents: "共享智能体",
      noSharedAgents: "暂无共享的智能体",
      sharedAgentsDesc: "已共享到本共享空间的智能体，参与空间可在对话中使用",
      sharedAgentsKbHint: "智能体绑定的知识库仅在参与空间使用该智能体对话时可 {'@'} 使用（只读），不会出现在「知识库列表」中。若需参与空间在列表中看到或编辑知识库，请单独将知识库共享到本空间。",
      ...
      permissionCalcFormula: "共享权限为共享时给参与空间设定的权限；我的实际权限 = min(共享权限, 当前空间在本共享空间中的角色)",
      permissionCalcTip: "我的实际权限 = min(共享权限, 当前空间在本共享空间中的角色)。当前空间在共享空间内是只读时，对此知识库最多只读；是编辑或管理员时，不超过共享权限。",
      inviteMembers: "加入方式",
      inviteMembersDesc: "通过邀请码或链接让其它园所空间加入本共享空间",
      inviteLink: "邀请链接",
      ...
      removeShareConfirm: "确定将「{name}」从本共享空间中移除？移除后参与空间将无法再访问该知识库。",
      removeAgentShareConfirm: "确定将「{name}」从本共享空间中移除？移除后参与空间将无法再访问该智能体。",
      removeShareSuccess: "已从共享空间中移除",
      removeShareFailed: "移除失败，请重试",
      requireApproval: "需要审核",
      requireApprovalDesc: "开启后，新的参与空间加入需要管理员审核",
      searchable: "开放可被搜索",
      searchableDesc: "开启后，共享空间将出现在「加入共享空间」的搜索列表中，其它园所空间可搜索并申请加入，无需邀请码",
      memberLimit: "参与空间数量上限",
      memberLimitDesc: "超过上限后无法再添加新的参与空间；设为 0 表示不限制",
      memberLimitPlaceholder: "0 表示不限制",
      memberLimitHint: "当前参与空间数：{count}",
      joinRequests: "加入申请",
      joinRequestsDesc: "审核待加入本共享空间的申请",
    ...
    editor: {
      ...
      navJoin: "加入共享空间",
      basicTitle: "基本信息",
      basicDesc: "设置共享空间的名称和描述，便于参与空间识别",
      nameTip: "建议使用集团、区域或协作项目名称，便于参与空间识别",
      descriptionTip: "描述共享空间的用途和目标，帮助参与空间了解边界",
      permissionsTitle: "共享权限",
      permissionsDesc: "了解参与空间不同角色对知识库与智能体的权限范围",
      permissionFeature: "权限功能",
      fullAccess: "完整权限",
      editAccess: "编辑权限",
      viewAccess: "只读权限",
      adminPerm1: "管理共享空间设置、参与空间及知识库与智能体共享",
    ...
    addMember: {
      button: "添加参与空间",
      dialogTitle: "添加参与空间",
      tip: "从用户列表选择代表用户；系统会按该用户所属工作空间授权，同一工作空间下的用户共享访问权限。",
      tipTenant: "共享空间的参与单位是空间：选定一个空间后，其下全部用户都将获得访问权限。下方搜索结果按空间去重。",
      searchUser: "选择代表用户",
      searchTenant: "选择空间",
      searchPlaceholder: "输入用户名或手机号搜索...",
      searchTenantPlaceholder: "输入空间名、用户名或手机号搜索...",
      searchHint: "可直接从列表选择，输入至少 2 个字符可按用户名或手机号搜索",
      searchTenantHint: "输入至少 2 个字符开始搜索，按空间去重并自动过滤已加入的空间",
      alreadyInSpace: "所属空间已加入",
      selectRole: "分配角色",
      confirmBtn: "添加",
      success: "参与空间添加成功",
      failed: "添加失败",
    ...
    share: {
      ...
      modelShareDesc: "将模型共享到共享空间，参与空间可使用该模型",
      agentShareDesc: "将智能体共享到共享空间，参与空间可使用该智能体",
      spaceAgentShareCountTip: "该共享空间的共享智能体数量",
      selectOrg: "选择共享空间",
      selectOrgPlaceholder: "请选择要共享到的共享空间",
      permission: "权限",
      permissionTip: "可编辑权限允许参与空间修改知识库内容，只读权限仅允许检索和问答",
      shareSuccess: "已共享",
      shareFailed: "共享失败",
  ...
  tenantMember: {
    title: "成员管理",
    sectionDescription:
      "邀请伙伴加入当前空间并分配角色。Owner 和 Admin 可管理普通成员；Owner 角色变更仅限 Owner 或系统管理员。",
    ...
    audit: {
      ...
      action: {
        ...
        "rbac.member_suspended": "挂起成员",
        "rbac.member_reactivated": "恢复成员",
