...
test('memory editor exposes service and sprout controls for every memory type', () => {
  ...
  assert.ok(source.includes('listServiceReminders'))
  assert.ok(source.includes('mapServiceReminderToTask'))
  assert.ok(source.includes('linkedServiceReminderTask'))
  ...
  assert.ok(source.includes("return '重新提取服务'"))
  assert.ok(!source.includes("if (linkedServiceReminderTask.value) {\n    noteActiveTab.value = 'service'\n    return\n  }"))
  assert.ok(source.includes('noteServiceSummaryLabel'))
  assert.ok(source.includes('noteServiceDisplayTitle'))
  assert.ok(source.includes('noteServiceHighlightTitle'))
  assert.ok(source.includes('关键信息'))
  assert.ok(source.includes('来源关联'))
  assert.ok(source.includes('暂无服务资料'))
  assert.ok(source.includes('加载服务卡片中'))
  ...
  assert.ok(source.includes('loadLinkedServiceReminder(item.id)'))
  ...
  assert.ok(source.includes('v-if="audioSourceCardVisible" class="memory-audio-panel memory-audio-panel--source"'))
  assert.ok(source.includes(':src="audioPlayerUrl" controls preload="metadata"'))
  assert.ok(source.includes('const audioSourceCardVisible = computed(() => isAudioMemory.value && Boolean(audioSourceUrl.value))'))
  assert.ok(source.includes('normalizeAuthenticatedFileProxyUrl'))
  assert.ok(source.includes("return source.replace(/^\\/api\\/v1\\/files(?=\\?)/, '/files')"))
  assert.ok(source.includes('asTrimmedString(metadata.audio_file_path)'))
  assert.ok(source.includes('const audioPlayerUrl = ref'))
  assert.ok(source.includes('const rawBlob = await getDown(sourceUrl)'))
  assert.ok(source.includes('const blobUrl = URL.createObjectURL(blob)'))
  assert.ok(source.includes('revokeAudioPlayerObjectUrl'))
...
test('memory service tab uses shared weak extraction rules', () => {
  ...
  assert.ok(serviceExtractionSource.includes('hasCustomerIdentity'))
  assert.ok(serviceExtractionSource.includes('subjectName'))
  assert.ok(serviceExtractionSource.includes('serviceMode'))
